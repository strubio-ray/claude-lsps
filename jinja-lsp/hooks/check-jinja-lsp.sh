...
if ! command -v cargo &>/dev/null; then
  echo "[$BINARY] Rust/Cargo not found. Install: https://rustup.rs"
  exit 1
fi

# Serialized cargo install (flock with mkdir fallback for macOS)
do_install() {
  echo "[$BINARY] Installing via Cargo..."
  if cargo install "$BINARY"; then
...
if ! command -v "$BINARY" &>/dev/null; then
  echo "[$BINARY] Not in PATH after install. Install manually: cargo install $BINARY"
  exit 1
fi
